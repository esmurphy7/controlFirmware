...
void setup() {
  ...
  Serial.list();
  port = new Serial(this,Serial.list()[1], 115200); // joey, switch [1] to [0] to run it without an arduino
  ...
  calibrateButton = new Button(20,390,16,color(150,150,150),color(12),"Calibrate","Calibrate",font);
...
void draw() {
  ...
  if(calibrated == false) {
    text("Uncalibrated",120,405);
  }
  if(calibrated == true) {
    text("Calibrated",120,405);
  }
  ...
  if(startButton.getValue() == true) {
    ...
    if(controlToggle.getValue() == true) {
      for(int i=0;i<5;i++) {
        ...
        if(trig < 0) {
          setpoints[i] = floor(127+127*Rturn*amplitude.value()*trig);
        }
        else {
          setpoints[i] = floor(127+127*Lturn*amplitude.value()*trig);
        }
        if(keys[2] == true) {
          t+=0.17;
        }
...
void mousePressed() {
  ...
  if(hitColor == color(12)) {
    calibrateButton.toggle();
    port.write(byte(254));
    port.write(byte(254));
    port.write(byte(0));    
    port.write(byte(0));    
